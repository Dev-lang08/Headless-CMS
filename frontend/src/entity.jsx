...
function Entity() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.put(`/api/entities/${entityName}/${selectedRow[primaryKey]}`, formData);
      fetchEntityData(entityName);
      setFormData({});
      setSelectedRow(null);
    } catch (error) {
  ...
  return (
    <div>
            ...
            <tbody>
              {entityData.map(row => (
                <tr key={row[primaryKey]}>
                  ...
                  <td>
                    <td><button className='btn btn-danger' onClick={() => handleDelete(row[primaryKey])}>Delete</button></td>
                    <td><button className='btn btn-secondary' onClick={() => handleModify(row[primaryKey])}>Modify</button></td>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className='container'>
          <Link to="/">
            <button className='btn btn-danger' onClick={deleteEntity}>Delete Entity</button>
          </Link>
        </div>
