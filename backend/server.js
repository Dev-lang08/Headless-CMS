...
connection.connect(function(err) {
  ...
  connection.query("CREATE DATABASE if not exists vahan", function (err, result) {
    if (err) throw err;
...
app.put('/api/entities/:entityName/:id', async(req, res) => {
  ...
  // console.log("hi2");
  const query = `UPDATE ${entityName} SET ${setClause} WHERE ${key} = ?`;
  //console.log(query, id);
  connection.query(query, [id], (error, results, fields) => {
    if (error) {
      console.error(`Error updating row ${id} in ${entityName}:`, error);
      res.status(500).json({ error: `Error updating row ${id} in ${entityName}` });
    } else {
...
app.delete('/api/entities/:entityName/:id', async (req, res) => {
  ...
  try {
    ...
    const query = `DELETE FROM ${entityName} WHERE ${key} = ?`;
    connection.query(query, [id], (error, results, fields) => {
      if (error) {
        console.error(`Error deleting row ${id} from ${entityName}:`, error);
        res.status(500).json({ error: `Error deleting row ${id} from ${entityName}` });
      } else {
        res.status(200).json({ message: `Row ${id} deleted from ${entityName}` });
      }
    });
  } catch (error) {
    console.error(`Error fetching columns for ${entityName}:`, error);
